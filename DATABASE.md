...
`MessageAttachment`. Indexes were chosen for the query patterns its owning phase will need, same "cheap to add ahead of time, expensive to retrofit onto a populated table later" principle already applied to every other table before its phase landed (`Task`'s indexes existed before Phase 2, `Event`/`Reminder`'s before Phase 3, `Conversation`/`Message`'s before Phase 5).
