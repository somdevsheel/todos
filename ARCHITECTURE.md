...
**What exists today (Phases 1–3):** the NestJS API (REST only — no WebSocket gateway yet), PostgreSQL with the complete schema, a BullMQ-backed reminder worker running *inside* the API process (not yet split into its own `workspace-worker` container — see Phase 8), and the Next.js web app, all running locally via `docker-compose.dev.yml`. Nothing here has been deployed to Lightsail, Vercel, or any live domain — see [DEPLOYMENT.md](./DEPLOYMENT.md), [LIGHTSAIL.md](./LIGHTSAIL.md), [VERCEL.md](./VERCEL.md).
